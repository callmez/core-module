...
@section('title', app_name() . ' | ' . __('labels.admin.access.users.management'))
        ...
        <div class="row">
            <div class="col-sm-5">
                <h4 class="card-title mb-0">
                    {{ __('labels.admin.access.users.management') }} <small class="text-muted">{{ __('labels.admin.access.users.active') }}</small>
                </h4>
            </div><!--col-->

            <div class="col-sm-7">
                @include('admin.auth.user.includes.header-buttons')
            </div><!--col-->
        </div><!--row-->
                    ...
                    <table class="table">
                        ...
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td class="btn-td">@include('admin.auth.user.includes.actions', ['user' => $user])</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
